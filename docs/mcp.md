...
html.contact has a private-preview Model Context Protocol (MCP) integration for coding agents. It can search committed product documentation, inspect and diagnose forms and account capacity, review bounded form markup, create or configure an owned form with confirmation, return safe installation HTML, run one clearly synthetic activation test, and—with separate consent—read recent submissions.
...
| `forms:write` | Create a form, replace its complete allowed-domain list, update basic form settings, and send a synthetic test. |
...
The integration cannot manage payments or subscriptions, account deletion, API keys, linked emails, recipient addresses, form deletion, submission deletion, or arbitrary outbound email. It never returns payment records, raw network/security metadata, provider message identifiers, or private attachment storage keys.

## Tools

The live server metadata is the source of truth. Production currently defines exactly these fifteen tools:
...
| `configure_form_domains` | `forms:write` | Replaces the complete domain list after `REPLACE DOMAINS` confirmation. It requires the complete current domain list; stale snapshots fail with `form_state_conflict` instead of overwriting newer changes. |
...
Write calls use idempotency keys where retrying could otherwise duplicate work. The agent should show the proposed change and obtain the exact confirmation immediately before each mutation. Domain replacement and basic-settings updates require the complete previously read state so concurrent edits fail safely. `configure_form_settings` cannot change recipients, domains, redirects, direct-post policy, message defaults, deletion state, billing, or credentials.
...
6. Let the agent fetch installation instructions and add only the public `hc_pub_` action to frontend HTML.
7. Give `review_form_code` only the relevant form markup and exact deployed hostname. Do not include unrelated files, secrets, tokens, credentials, or private `hc_live_` keys. Let the coding agent apply the returned findings in the project's existing framework; the tool intentionally does not generate framework-specific code.
8. Verify the rendered page in a browser on an allowed domain.
9. Run `diagnose_form` with the deployed source hostname or observed public error code before changing configuration.
10. Before a synthetic test, review the usage and real-email warning, confirm it, then ask the agent to check the returned status.
...
For delivery-status meanings and browser form problems, continue with [HTML Forms](/docs/html-forms/), [Allowed Domains & Spam](/docs/domains-and-spam/), and [Troubleshooting](/docs/troubleshooting/). See the [Privacy Policy](/legal/privacy/) for the product's data handling terms.
